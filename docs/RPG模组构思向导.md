...
功能按钮分栏使用每项动作的 `group` 字段。功能按钮构思、抽取、整套生成和动作批量生成都会在自定义或默认模板后追加分栏字段约定，保留作者确认的分栏名与按钮归属；预览明确标出「未分栏」。构思和抽取调用 AI；预览中最终点击回填后，只按勾选草案执行代码校验和数据库写入，不再调用 AI。没有抽取出来的分栏不能由回填代码还原，可回到功能按钮步骤重新抽取，或在动作编辑器中填写分栏。

向导顶部提供「切换步骤」，可直接返回第 3 步（RPG 为「数值系统」，模拟器为「状态与资源」，养成为「成长指标」），切换时保留对话和草案，不自动调用模型。预览中每项数值可用「移至关系数值／移至玩家数值」调整归属，保留初值、范围和作用，并同步浏览器草案及确认记录；目标组重名时阻止覆盖。归属调整不会擅自改写动作效果或角色起点，回填前需核对相关引用，角色卡中可启用关系数值。

数值归属：数值构思、数值抽取和整套生成会在所选提示词之后追加字段归属约定，自定义模板同样生效。玩家自身或全局共用的指标归 `stat_defs`，每个 NPC 各自记录的关系与状态归 `relation_stat_defs`。模型需逐项标注 `scope=player/relation`；清洗按明确标记分类，放错数组会纠正并在预览中提示，未知标记会报错。兼容未标注的旧输出，但不根据“体力”“压力”等名称擅自推断归属。重新抽取数值步骤时，两组草案一起替换，空组也会清空旧草案，避免纠正后残留在原组。已有模组数据仍由用户预览回填，不自动改写。
...
### 回填的覆盖语义（2026-09-18 补）

原先只有「追加 + 同名跳过」。这在「手写模组拿向导补几个 NPC」下是对的，但在
**「整套生成 → 不满意 → 重新生成 → 再回填」**下会叠出两套：新方案的名字基本
都是新的，「同名跳过」等于没生效。

现在靠 `rpg_modules.wizard_state` 这份台账把两者分开——它记着**上一轮向导写了
哪些**（时段名、数值名、四张子表建出来的行 id）。回填时：

- 每一栏**先删掉台账里那一栏的行，再写这一轮的**；作者手写的不在台账里，一直留着
- 没勾的栏**一律不碰**：既不清空也不追加，旧行和台账都留着
- 文本栏仍然是直接覆盖（本来就没有叠加问题）
- 台账在每次回填后整份写回，含义是「现在这个模组里属于向导的东西」

删的细节：地点 `parent_id` 是硬外键，**子必须先于父**删（建的那边按层建，
同一个理由）；台账里已经被作者手删掉的 id 当成功，保证「回填失败重试」不会
卡住。

`wizard_state` 是老库读出来是 `{}`，那时按「没有台账」处理，行为退回加这一列之前。
