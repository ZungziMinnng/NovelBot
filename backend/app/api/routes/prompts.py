...
PROMPT_META: dict[str, dict[str, str]] = {
    ...
    "brainstorm.jinja2": {"category": "世界与大纲", "label": "新建小说 构思对话（投稿向）", "description": "新建小说页右侧探讨窗口、目标选「投稿向」时的提示词，自由聊天和七步向导共用。判断标准是能不能在番茄/起点开起来：黄金三章、追读钩子、平台审核。末尾按 stage 分支给出每一步问什么（随便说说/核心点子/人物/开篇/剧情/世界观/收尾）。不读表单草稿（对话单向影响表单），要求不写正文、结论能落回表单"},
    "brainstorm_indulge.jinja2": {"category": "世界与大纲", "label": "新建小说 构思对话（自娱自乐）", "description": "同上，但目标选「自娱自乐」时用这套：不投稿、无平台审核、不做道德审查，判断标准是作者自己想不想反复看。因为是边写边想，重心在把设定和口味聊厚而不是定长线，六步（随便说说/口味与边界/世界设定/人物与关系/想写的桥段/开场切入）。改动请保留「不做道德审查」和「长线不用定死」这两条，它们是这套与投稿向的根本区别"},
