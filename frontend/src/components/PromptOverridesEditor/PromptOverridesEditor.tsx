...
import type { UserPrompt, UserPromptsApi } from '@/api/userPrompts'
import { confirmDialog } from '@/components/ConfirmDialog/ConfirmDialog'

/**
 * 每用户提示词覆写的编辑器。RPG 和酒馆共用这一份。
 *
 * 两边只差 api 客户端、默认选中的模板、说明文案和 textarea 的 id；RPG 侧外面
 * 还套了页头和返回按钮，那部分留在页面里，不塞进来。
 */
interface Props {
  api: UserPromptsApi
  queryKey: unknown[]
  /** 默认选中哪个模板，如 rpg_gm.jinja2 */
  initialName: string
  intro: ReactNode
  /** 「模板变量与写法」折叠块里那句说明 */
  variablesHint: ReactNode
  textareaId: string
  /** 加载中/加载失败时要不要仍然显示 intro——RPG 是独立页面要显示，酒馆是嵌入面板不显示 */
  introWhilePending?: boolean
  onDirtyChange?: (dirty: boolean) => void
}

export default function PromptOverridesEditor({
  api, queryKey, initialName, intro, variablesHint, textareaId,
  introWhilePending = false, onDirtyChange,
}: Props) {
  ...
  const accept = (updated: UserPrompt) => {
    queryClient.setQueryData<UserPrompt[]>(queryKey, current =>
  ...
  const save = async () => {
    ...
    setError('')
    try {
      accept(await api.update(selected.name, content))
  ...
  const reset = async () => {
    ...
    setError('')
    try {
      accept(await api.reset(selected.name))
  ...
  const introBlock = <p className="text-sm text-muted-foreground leading-relaxed">{intro}</p>

  if (isLoading) return (
    <div className="space-y-4">
      {introWhilePending && introBlock}
    ...
    <div className="space-y-4">
      ...
      {selected && (
        <div className="border rounded-lg p-4 bg-card/70 space-y-4">
          ...
          <label htmlFor={textareaId} className="block text-sm font-medium">{selected.label}提示词</label>
          <textarea
            id={textareaId}
            value={content}
